...
class HRNetModel(ISModel):
    ...
    def __init__(self, width=48, ocr_width=256, small=False, backbone_lr_mult=0.1,
                 norm_layer=nn.BatchNorm2d, is_att=True, net_mode='', **kwargs):
        super().__init__(norm_layer=norm_layer, **kwargs)

        self.feature_extractor = HighResolutionNet(width=width, ocr_width=ocr_width, small=small,
                                                   num_classes=1, norm_layer=norm_layer, is_att=is_att)
    ...
    def backbone_forward(self, image, coord_features=None, points=None):
        net_outputs = self.feature_extractor(image, coord_features, points)
