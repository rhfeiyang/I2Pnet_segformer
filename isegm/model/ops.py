...
class DistMaps(nn.Module):
    ...
    def get_coord_features(self, points, batchsize, rows, cols):
        if self.cpu_mode:
            ...
            # filter outlier point because of zoom-in
            for p_index in range(points.shape[0]):
                if points[p_index][0] >= rows or points[p_index][1] >= cols:
                    invalid_points[p_index] = True
