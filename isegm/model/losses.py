...
class ClickLoss(nn.Module):
    def __init__(self, tau=100 / 255, alpha=0.8, beta=2.0, from_sigmoid=False, eps=1e-9):
    ...
    def forward(self, pred, label, distance_map, sample_weight=None):
        if not self._from_sigmoid:
            pred = torch.sigmoid(pred)

        binary_loss = self.criterion(pred, label)
        ones = torch.ones_like(label)

        pos_point_map = distance_map[:, 0:1]
        pos_dm = 1 - torch.min(pos_point_map / self.tau, ones)
        pos_weight = self.alpha + pos_dm * (self.beta - self.alpha)

        neg_point_map = distance_map[:, 1:]
        neg_dm = 1 - torch.min(neg_point_map / self.tau, ones)
        neg_weight = self.alpha + neg_dm * (self.beta - self.alpha)
        # w = gt*pos_weight + (gt==0)*neg_weight
        w = torch.where(label == 1, pos_weight, neg_weight)

        w[label == 255] = 0
        # w = w / (w.sum() + self._eps)
        loss = torch.mean(w * binary_loss)
        return loss


class ZoomInLoss(nn.Module):
    def __init__(self):
        super(ZoomInLoss, self).__init__()
        self.NFL = NormalizedFocalLossSigmoid(alpha=0.5, gamma=2)

    def forward(self, zoom_in_pred, label):
        output = zoom_in_pred['output']
        output = F.interpolate(output, size=label.size()[2:], mode='bilinear', align_corners=True)
        return self.NFL(output, label)
