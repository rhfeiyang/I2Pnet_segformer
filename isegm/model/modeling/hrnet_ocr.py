...
class HighResolutionNet(nn.Module):
    def __init__(self, width, num_classes, ocr_width=256, small=False,
                 norm_layer=nn.BatchNorm2d, align_corners=True, is_att=True):
    ...
    def forward(self, x, additional_features=None, points=None):
        feats, low_level = self.compute_hrnet_feats(x, additional_features, points)
        if self.ocr_width > 0:
            out_aux = self.aux_head(feats)

            feats = self.att(feats, points)
            feats = self.upatt(feats, low_level, points)
            feats = self.conv3x3_ocr(feats)

            out_aux = F.interpolate(out_aux, size=feats.size()[2:], mode='bilinear', align_corners=True)
    ...
    def compute_hrnet_feats(self, x, additional_features, points):
        low_level, x = self.compute_pre_stage_features(x, additional_features)
        x = self.layer1(x)
        x_list = []
        ...
        x = self.aggregate_hrnet_features(x)
        # x = self.att(x, points)
        # x = self.upatt(x, low_level, points)
    ...
    def compute_pre_stage_features(self, x, additional_features):
        ...
        if additional_features is not None:
            low_x = x + additional_features
        x = self.conv2(low_x)
        x = self.bn2(x)
        return low_x, self.relu(x)
