...
from isegm.utils.exp import load_config_file, str2bool
...
def parse_args():
    ...
    parser.add_argument('--mode', choices=['NoBRS'], default='NoBRS', help='')

    parser.add_argument('--intention', type=str, default='', required=True,
                        help='the checkpoint path of user intention network')

    group_checkpoints = parser.add_mutually_exclusive_group(required=True)
    group_checkpoints.add_argument('--segmentation', dest='checkpoint', type=str, default='')
    ...
    parser.add_argument('--datasets', type=str, default='GrabCut,Berkeley,DAVIS,COCO_MVal,PascalVOC,SBD,COCO_MVal_seen,COCO_MVal_unseen')
    ...
    parser.add_argument('--zoom-in', dest='zoom_in_switch', type=int, help='Open: >0  Close: <=0', default=1)
    parser.add_argument("--att", type=str2bool, default=True)
    parser.add_argument('--ig-bd', dest='ignore_boundary', default=False, action='store_true', help='IoU ignore boundary of 1px')
    parser.add_argument('--eval--mode', dest='evaluation_mode', type=str, help='evaluation mode: normal / cascade', default='normal')
    parser.add_argument('--segnet--mode', dest='segnet_mode', type=str, default='att+upatt', help='network mode')
    parser.add_argument('--intnet--mode', dest='intnet_mode', type=str, default='att+upatt', help='network mode')
    parser.add_argument('--empty-gpus', dest='empty_gpus')
    ...
    if args.cpu:
        ...
        if args.gpus == 'auto':
            try:
                free_gpus = np.array(py3nvml.get_free_gpus()).astype('int')
                if args.multi_processing_sign:
                    args.empty_gpus = np.where(free_gpus == 1)[0].tolist()
                free_index = str(np.where(free_gpus == 1)[0].tolist()[0])
                args.device = torch.device(f"cuda:{free_index}")
            except:
    ...
    if args.ignore_boundary:
        green_highlight(f'[== Ignore Boundary ==] activated!')
...
def main():
    args, cfg = parse_args()
    perform_evaluation(args, cfg)


def perform_evaluation(args, cfg):
    ...
    for dataset_name in args.datasets.split(','):
        ...
        for checkpoint_path in checkpoints_list:
            int_checkpoint_path = Path(args.intention)
            int_model = utils.load_is_model(int_checkpoint_path, args.device, net_mode=args.intnet_mode)
            seg_model = utils.load_is_model(checkpoint_path, args.device, net_mode=args.segnet_mode)

            predictor_params, zoomin_params = get_predictor_and_zoomin_params(args, dataset_name)
            zoomin_params = zoomin_params if args.zoom_in_switch > 0 else None

            predictor = get_predictor(int_model, seg_model, args.mode, args.device,
                                      prob_thresh=args.thresh,
                                      predictor_params=predictor_params,
                                      zoom_in_params=zoomin_params,
                                      eval_mode=args.evaluation_mode)

            ignore_boundary = False if dataset_name == 'PascalVOC' else args.ignore_boundary
